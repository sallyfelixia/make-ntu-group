...
def LCD_show(input_stringl, input_string2) :
    lcd.clear()
    lcd.cursor_pos = (0,0)
...
try:
    RECORD = 0
    LCD_show( 'Please select your mode.','A:Food B:Clothe C:Living D:Vehicle')
    while(True):
        if( Kb_Detect() ):
            if( Cur_j == 3 ):
                if ( Cur_i == 0 ):
                ...
                LCD_show( 'Please select your mode first.','Good luck!')
